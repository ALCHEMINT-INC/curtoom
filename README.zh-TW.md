...
**問題** — 筆電上播得好好的 hero 片到手機上一頓一頓：1792×3184、CRF 20 壓出 11.7 Mbps／69 MB，約 7.5 Mbps 的手機線路餵它一秒播一秒等。

**什麼時候用** — 任何要上手機的 9:16 背景／hero 片；已經在手機上卡的片。

**它做什麼** — 一條指令。母帶已經合規（H.264、≤ 1440×2560、≤ 6 Mbps）就只 remux；否則 1440×2560、CRF 23、preset slow、音軌原封複製。印每秒碼率和 `PASS` 或 `FAIL: rerun with 1920`。

**為什麼是 CRF 23** — CRF 鎖的是感知品質不是碼率：同一個 CRF 20 靜態鏡頭 6 Mbps、高運動量 8.6——而 8.6 就是卡。CRF 23 落在 5.7，手機大小的背景片看不出差別。高於 1440×2560 一律不留，手機螢幕最寬 1290×2796。

**效益** — 69 MB → 34.5 MB、11.7 → 5.7 Mbps，Fast 4G 下一邊預載下一支一邊全程順播。

<sub>驗證於 2026-09-03 · ffmpeg 8.0 · macOS 26</sub>

### VERIFY · [`verifying-hero-video-on-mobile`](skills/verifying-hero-video-on-mobile)

**問題** — 手機上 hero 不動，分不出是**餓死**（碼率高過頻寬）還是**被擋**（自動播放政策、低耗電模式、舊分頁）。桌機 Chrome 播得動不算證據。

**跟 ENCODE 的差別** — 不碰檔案。ENCODE 產片；VERIFY 回答「頁面在手機上播不播得出來、不播是為什麼」。獨立，各用各的。

**什麼時候用** — 選用。第 0 層一律做（一分鐘）；回報卡頓或想在沒手機時先拿證據才做第 1 層；回報「沒自動播放」「有播放鍵」才做第 2 層——而且第一步是請他關分頁重開。

**它做什麼** — 第 0 層：碼率門檻、`curl --limit-rate` 快篩、ETag＝MD5 與前 1.5 MB 驗 faststart、先上傳再 push、CDN range 快取的坑。第 1 層：Chrome DevTools throttle 測試台，用 `Math.random` 強制抽片。第 2 層：iOS 模擬器冷載入與切 app 截圖比對。

**效益** — 五分鐘內用證據把「頁面有 bug」和「裝置狀態」分開。

<sub>驗證於 2026-09-03 · Chrome 140 · iOS 26 模擬器</sub>

### STITCH · [`video-clip-stitching`](skills/video-clip-stitching)

**問題** — 鏈式生成——第 N 段的尾幀當第 N+1 段的首幀——每段續生的開頭都有約半秒幾乎不動的「死水」。直接 `concat` 看起來就是「卡住」「重複」；沒有一幀重複，是動作斷了。容器幀率也會騙人。

**什麼時候用** — Dreamina、Runway、Higgsfield、Seedance 這類 01／02／03 分段生成的系列；任何「卡住／重複／不順／overlap」的抱怨；生成素材的驗收。

**它做什麼** — `clip_qc.py` 回報三種幀率，用相位結構抓灌水幀（灌水 7.5× vs 真 1.1×）、停滯幀、預覽版混進高清版。`seam_probe.py` 把接縫分類——不同鏡位 → 硬切；構圖延續 → 砍死水＋溶接；真重疊 → 砍掉被重演的段落。`join_clips.sh` 一次編碼，`xfade` offset 自動算，三個 ffmpeg 地雷事先拆掉。

**為什麼是比值** — 切點是「接縫後運動量 ÷ 接縫前運動量」趨近 1 的地方（低於 0.75 是死水，高於 1.4 是暴衝）。只看曲線曾經說要砍 54 幀，正解是 0。
...
沒有一個是計畫好的。官網的 hero 片是一隻貓跟著發光的紙船走進夜晚的街道，分三段生成，每段從前一段的最後一幀起頭。兩個晚上，這一段素材長出了三支 skill，順序如下。

**STITCH，2026-09-01 晚上。** 原話差不多是：「Downloads 裡有 cat mobile 01、02、03，03 的前幾幀跟 02 的尾巴好像有 overlap，你看一下怎麼處理，最後把 1、2、3 連起來合成一支。」十四分鐘後成片回來了，回覆是「欸？我以為可以用 FFmpeg 處理啊？……我看了一下，還不錯啊！」——全程就是 ffmpeg。下一句就是「有啥 skill 可以創造嗎」。中間還有：幀率驚魂（「原本不是 60 嗎？為什麼出來 30？」——剪映顯示的是它自己的 30 fps 時間軸，檔案是真 60）、02 被量出 13.4% 停滯幀、Downloads 裡的 desktop 03 其實是 854×480 預覽版、還有一次信心滿滿的「砍 54 幀」——正解是 0。

**ENCODE，隔天。** 接好的片——1792×3184、19 Mbps——原尺寸進了官網手機輪播；commit 說解析度標準是下限。CRF 20 壓成 11.7 Mbps／69 MB。隔天早上：「之前行動端會自動播放，現在卡在那裡。是哪裡出問題，還是我手機的問題？」原因查清後：「所以我現在全部都要重壓是吧？……不對，是你這邊來重新壓縮是吧？」一支檔。這段對話就是規格：一條指令、不用做決定。

**VERIFY，同一個早上。** 修正版 04:47 上線。接著：「還是沒有自動播放」——「我看到的是播放按鈕」——「移動 app Chrome」——「Safari 倒是沒問題」——「Chrome 連點了播放鍵都沒反應。。。」——「沒事」——「突然好了。」二十分鐘；bug 是一個還在跑昨天頁面的 Chrome 分頁。同一時間 iOS 模擬器已經冷載入網站、切 app 再切回來，證明頁面會自動播放也會續播。沒有人去「修」那段根本沒壞的 visibility 邏輯，檢查表多了最有用的兩行：先請對方關分頁重開；線上影片絕不同名覆蓋。
