...
**问题** — 笔记本上播得好好的 hero 片到手机上一顿一顿：1792×3184、CRF 20 压出 11.7 Mbps／69 MB，约 7.5 Mbps 的手机线路喂它一秒播一秒等。

**什么时候用** — 任何要上手机的 9:16 背景／hero 片；已经在手机上卡的片。

**它做什么** — 一条命令。母带已经合规（H.264、≤ 1440×2560、≤ 6 Mbps）就只 remux；否则 1440×2560、CRF 23、preset slow、音轨原样复制。打印每秒码率和 `PASS` 或 `FAIL: rerun with 1920`。

**为什么是 CRF 23** — CRF 锁的是感知质量不是码率：同一个 CRF 20 静态镜头 6 Mbps、高运动量 8.6——而 8.6 就是卡。CRF 23 落在 5.7，手机大小的背景片看不出差别。高于 1440×2560 一律不留，手机屏幕最宽 1290×2796。

**收益** — 69 MB → 34.5 MB、11.7 → 5.7 Mbps，Fast 4G 下一边预载下一支一边全程流畅。

<sub>验证于 2026-09-03 · ffmpeg 8.0 · macOS 26</sub>

### VERIFY · [`verifying-hero-video-on-mobile`](skills/verifying-hero-video-on-mobile)

**问题** — 手机上 hero 不动，分不清是**饿死**（码率高过带宽）还是**被挡**（自动播放策略、低电量模式、旧标签页）。桌面 Chrome 播得动不算证据。

**跟 ENCODE 的区别** — 不碰文件。ENCODE 产片；VERIFY 回答「页面在手机上播不播得出来、不播是为什么」。独立，各用各的。

**什么时候用** — 选用。第 0 层一律做（一分钟）；反馈卡顿或想在没手机时先拿证据才做第 1 层；反馈「不自动播放」「有播放键」才做第 2 层——而且第一步是请他关标签页重开。

**它做什么** — 第 0 层：码率门槛、`curl --limit-rate` 快筛、ETag＝MD5 与前 1.5 MB 验 faststart、先上传再 push、CDN range 缓存的坑。第 1 层：Chrome DevTools 限速测试台，用 `Math.random` 强制抽片。第 2 层：iOS 模拟器冷加载与切 app 截图比对。

**收益** — 五分钟内用证据把「页面有 bug」和「设备状态」分开。

<sub>验证于 2026-09-03 · Chrome 140 · iOS 26 模拟器</sub>

### STITCH · [`video-clip-stitching`](skills/video-clip-stitching)

**问题** — 链式生成——第 N 段的尾帧当第 N+1 段的首帧——每段续生的开头都有约半秒几乎不动的「死水」。直接 `concat` 看起来就是「卡住」「重复」；没有一帧重复，是动作断了。容器帧率也会骗人。

**什么时候用** — Dreamina、Runway、Higgsfield、Seedance 这类 01／02／03 分段生成的系列；任何「卡住／重复／不顺／overlap」的抱怨；生成素材的验收。

**它做什么** — `clip_qc.py` 报告三种帧率，用相位结构抓灌水帧（灌水 7.5× vs 真 1.1×）、停滞帧、预览版混进高清版。`seam_probe.py` 把接缝分类——不同机位 → 硬切；构图延续 → 砍死水＋叠化；真重叠 → 砍掉被重演的段落。`join_clips.sh` 一次编码，`xfade` offset 自动算，三个 ffmpeg 地雷事先拆掉。

**为什么是比值** — 切点是「接缝后运动量 ÷ 接缝前运动量」趋近 1 的地方（低于 0.75 是死水，高于 1.4 是暴冲）。只看曲线曾经说要砍 54 帧，正解是 0。
...
没有一个是计划好的。官网的 hero 片是一只猫跟着发光的纸船走进夜晚的街道，分三段生成，每段从前一段的最后一帧起头。两个晚上，这一段素材长出了三个 skill，顺序如下。

**STITCH，2026-09-01 晚上。** 原话差不多是：「Downloads 里有 cat mobile 01、02、03，03 的前几帧跟 02 的尾巴好像有 overlap，你看一下怎么处理，最后把 1、2、3 连起来合成一支。」十四分钟后成片回来了，回复是「欸？我以为可以用 FFmpeg 处理啊？……我看了一下，还不错啊！」——全程就是 ffmpeg。下一句就是「有啥 skill 可以创造吗」。中间还有：帧率惊魂（「原本不是 60 吗？为什么出来 30？」——剪映显示的是它自己的 30 fps 时间轴，文件是真 60）、02 被测出 13.4% 停滞帧、Downloads 里的 desktop 03 其实是 854×480 预览版、还有一次信心满满的「砍 54 帧」——正解是 0。

**ENCODE，第二天。** 接好的片——1792×3184、19 Mbps——原尺寸进了官网手机轮播；commit 说分辨率标准是下限。CRF 20 压成 11.7 Mbps／69 MB。第二天早上：「之前移动端会自动播放，现在卡在那里。是哪里出问题，还是我手机的问题？」原因查清后：「所以我现在全部都要重压是吧？……不对，是你这边来重新压缩是吧？」一个文件。这段对话就是规格：一条命令、不用做决定。

**VERIFY，同一个早上。** 修正版 04:47 上线。接着：「还是没有自动播放」——「我看到的是播放按钮」——「移动 app Chrome」——「Safari 倒是没问题」——「Chrome 连点了播放键都没反应。。。」——「没事」——「突然好了。」二十分钟；bug 是一个还在跑昨天页面的 Chrome 标签页。同一时间 iOS 模拟器已经冷加载网站、切 app 再切回来，证明页面会自动播放也会续播。没有人去「修」那段根本没坏的 visibility 逻辑，检查表多了最有用的两行：先请对方关标签页重开；线上视频绝不同名覆盖。
